export class Range {
  ...
  setValue(value: number, triggerCallback = false): void {
    if (this.seek_lock) {
      return;
    }
  ...
  setPercentage(percentage: number, triggerCallback = false): void {
    if (percentage < 0) {
      percentage = 0;
    }
  ...
  private updateProgressPosition = (e: MouseEvent): void => {
    const rect = this.target.getBoundingClientRect();
